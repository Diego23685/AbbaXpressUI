...
export default function RecepcionCarga() {
  ...
  // Lista de paquetes con tarifa unitaria editable en vivo
  ...
  const cargarDatosIniciales = async () => {
    try {
      ...
      if (configRes.data) {
        ...
        // Actualizar tarifa inicial del primer paquete con base en configuración global
  ...
  const handleCambioCliente = (e) => {
    ...
    // Actualizar tarifas de los paquetes existentes según el nuevo cliente seleccionado
    if (encontrado) {
      setPaquetes(prev => prev.map(p => {
  ...
  const agregarPaquete = () => {
    // Heredar tarifa según el cliente actual o configuración global
  ...
  const actualizarPaquete = (id, campo, valor) => {
    setPaquetes(
      paquetes.map((p) => {
        ...
        // Si cambia la categoría o vía, autocalculamos una tarifa sugerida por defecto a menos que el usuario la modifique manualmente después
  ...
  const calcularSubtotalPaquete = (pkg) => {
    ...
    // Si es categoría fija (Celular), la tarifa actúa como monto fijo; si es general/marítimo/aéreo, es por libra
    if (pkg.categoria === 'CELULAR') {
      return tarifa;
    }
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6 font-sans text-slate-800">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Registro multipaquete, categorización inteligente y liquidación
          </p>
        </div>
        <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-200 text-xs">
          <span className="text-slate-500 font-medium">T/C Oficial:</span>
          <span className="font-display font-bold text-brand">C$ {tipoCambio.toFixed(2)} NIO</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {acordeonCliente && (
            <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
              ...
              {/* Selector de Sucursal Destino (SOLO se muestra si es B2B Inter-Sucursal) */}
              {clienteActual?.tipoCliente === 'SUCURSAL_B2B' && (
                <div>
        ...
        {/* Acordeón 2: Paquetes Dinámicos con Tarifas Editables en Vivo */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <button
            type="button"
              ...
              <div className="space-y-3">
                {paquetes.map((pkg) => (
                  ...
                  >
                    ...
                    {/* Tarifa Editable en Vivo por ítem */}
                    <div className="lg:col-span-1">
                      <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1" title="Tarifa ajustable en vivo">Tarifa ($)</label>
                      <input
                        type="number"
                        step="0.01"
                        min="0"
      ...
      {/* Modal de Ticket Térmico y Digital */}
      {ticketData && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          
          {/* Estilos estrictos para la impresora térmica (80mm) */}
          ...
          <div className="bg-white rounded-3xl max-w-sm w-full shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            {/* 1. TICKET DIGITAL (A color y corporativo para pantalla / WhatsApp) */}
            <div className="p-5 overflow-y-auto font-mono text-xs text-slate-800 space-y-3 bg-slate-50 select-text" id="ticket-digital-pantalla">
                ...
                {ticketData.paquetes.map((p, idx) => (
                  <div key={idx} className="text-[11px] space-y-0.5 bg-white p-2.5 rounded-lg border border-slate-200">
                    ...
                    <div className="text-[10px] text-slate-500 flex justify-between">
                      ...
                      <span>C$ {(p.subtotal * ticketData.tipoCambioAplicado).toFixed(2)}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="text-center text-[10px] text-slate-400 pt-1">
                ...
                <p>Conserve este comprobante para su retiro.</p>
              </div>
            </div>
            ...
            <div className="hidden" id="ticket-termico-impresion">
              ...
              <div className="space-y-1 border-b border-dashed border-black pb-2.5 text-[11px]">
                ...
                <p><strong>Condición:</strong> {ticketData.metodoPago === 'CREDITO' ? 'CREDITO (PENDIENTE)' : `PAGADO (${ticketData.metodoPago})`}</p>
              </div>
                ...
                {ticketData.paquetes.map((p, idx) => (
                  <div key={idx} className="text-[11px] py-1 border-b border-slate-300 last:border-0 space-y-0.5">
                    ...
                    <div className="flex justify-between text-[10px]">
                      <span>{p.label || 'General'} ({p.pesoLbs}lb - {p.viaEnvio})</span>
                      <span>C$ {(p.subtotal * ticketData.tipoCambioAplicado).toFixed(2)}</span>
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center text-[10px] pt-2 border-t border-dashed border-black space-y-0.5">
                <p>¡Gracias por preferir Abba Xpress!</p>
                <p>Conserve este comprobante para retiro.</p>
              </div>
            </div>
